...
export default class Menu {
    ...
    async create() {
      // Validar si se debe crear el storage de la collection
      this.comun.createColeccion();
      ...
      this.btn_jugar = this.relatedScene.add.sprite(this.cw / 2, 180, 'btn_jugar').setAlpha(1).setDepth(1).setScale(1.5);
      this.btn_coleccion = this.relatedScene.add.sprite(this.cw / 2, 280, 'btn_coleccion').setAlpha(1).setDepth(1).setScale(1.5);
      this.btn_reiniciar = this.relatedScene.add.sprite(this.cw / 2, 380, 'btn_reiniciar').setAlpha(1).setDepth(1).setScale(1.5);

      this.containerMenu = this.relatedScene.add.container(0, -500, 
        [this.menuContext, this.btn_jugar, this.btn_coleccion, this.btn_reiniciar]
      ).setDepth(10);
    ...
    showMenu(){
      ...
      this.relatedScene.tweens.add({
        ...
        onComplete: function () {
          ...
          self.btn_coleccion.setDepth(2).setInteractive({cursor:'pointer'});
          self.btn_reiniciar.setDepth(2).setInteractive({cursor:'pointer'});                                                    
        },
      });
    }
    ...
    interationButtons() {
      ...
      this.btn_jugar.on('pointerover', () => {
        this.relatedScene.tweens.add({
            ...
            scaleX: 1.7,      // Nuevo valor de escala en X
            scaleY: 1.7,      // Nuevo valor de escala en Y
      ...
      this.btn_jugar.on('pointerout', () => {
          this.relatedScene.tweens.add({
              targets: this.btn_jugar,
              scaleX: 1.5,      // Restaurar el valor original de escala en X
              scaleY: 1.5,      // Restaurar el valor original de escala en Y
              duration: 200,
              ease: 'Power1',
          });
      });
      ...
      this.btn_coleccion.on('pointerover', () => {
        this.relatedScene.tweens.add({
            targets: this.btn_coleccion,
            scaleX: 1.7,      // Nuevo valor de escala en X
            scaleY: 1.7,      // Nuevo valor de escala en Y
            duration: 200,    // Duración de la animación en milisegundos
            ease: 'Power1',   // Tipo de easing
      ...
      this.btn_coleccion.on('pointerout', () => {
          this.relatedScene.tweens.add({
              targets: this.btn_coleccion,
              scaleX: 1.5,      // Restaurar el valor original de escala en X
              scaleY: 1.5,      // Restaurar el valor original de escala en Y
      ...
      this.btn_coleccion.on('pointerdown', () => {
        this.containerMenu.setVisible(false);
        this.showCollection();
      });

      //INTERACION CON EL BOTON REINICIAR
      this.btn_reiniciar.on('pointerover', () => {
        this.relatedScene.tweens.add({
            targets: this.btn_reiniciar,
            scaleX: 1.7,      // Nuevo valor de escala en X
            scaleY: 1.7,      // Nuevo valor de escala en Y
            duration: 200,    // Duración de la animación en milisegundos
            ease: 'Power1',   // Tipo de easing
      ...
      this.btn_reiniciar.on('pointerout', () => {
          this.relatedScene.tweens.add({
              targets: this.btn_reiniciar,
              scaleX: 1.5,      // Restaurar el valor original de escala en X
              scaleY: 1.5,      // Restaurar el valor original de escala en Y
      ...
      this.btn_reiniciar.on('pointerdown', () => {
        this.comun.resetCollection();
      });
    }
